...
* `fpdfsdk/fpdf_edit_embeddertest.cpp` — new
  `FPDFEditEmbedderTest::SetFontSize` exercising three blocks
  (matching the multi-block layout of CL 131231 used as the
  reference template):
  1. Round-trip Get → Set → Get returns the new value through
     three sizes (12 → 24 → 0; zero is permitted to mirror
     `FPDFPageObj_NewTextObj` at size 0).
  2. Rejection paths preserve the prior value (negative size and
     null handle both `EXPECT_FALSE` while leaving the readback
     unchanged at 18).
  3. Persistence through `FPDF_SaveAsCopy` + `OpenSavedDocument` +
     `LoadSavedPage`: the new size survives the full save-and-
     reload pipeline, asserting the mutation actually reaches the
     PDF dictionary (not just the in-memory text-state). This is
     stronger than the reference CL's pure in-memory check.
