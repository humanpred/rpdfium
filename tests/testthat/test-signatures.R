...
# -- C-side doc-pointer shims (cpp_signature_contents / _byte_range) --
#
# The R API uses the per-handle shims (cpp_signature_contents_handle
# /  cpp_signature_byte_range_handle). The doc-pointer variants
# cpp_signature_contents(doc_ptr, index_zero) and
# cpp_signature_byte_range(doc_ptr, index_zero) live in src/signatures.cpp
# and are exposed only through `pdfium:::`. The tests below exercise both
# branches:
#   * out-of-bounds index -> FPDF_GetSignatureObject returns NULL ->
#     Rcpp::stop fires (the defensive "returned NULL" path).
#   * valid index -> populated /Contents / /ByteRange buffers are returned.

test_that("cpp_signature_contents reads the doc-pointer happy path", {
  doc <- pdf_doc_open(fixture_path("signed"))
  on.exit(pdf_doc_close(doc), add = TRUE)
  raw <- pdfium:::cpp_signature_contents(doc$ptr, 0L)
  expect_type(raw, "raw")
  expect_length(raw, 4L)
  expect_identical(as.integer(raw), c(0xDEL, 0xADL, 0xBEL, 0xEFL))
})

test_that("cpp_signature_contents errors on an out-of-bounds index", {
  doc <- pdf_doc_open(fixture_path("signed"))
  on.exit(pdf_doc_close(doc), add = TRUE)
  expect_error(
    pdfium:::cpp_signature_contents(doc$ptr, 99L),
    "FPDF_GetSignatureObject returned NULL"
  )
})

test_that("cpp_signature_byte_range reads the doc-pointer happy path", {
  doc <- pdf_doc_open(fixture_path("signed"))
  on.exit(pdf_doc_close(doc), add = TRUE)
  m <- pdfium:::cpp_signature_byte_range(doc$ptr, 0L)
  expect_true(is.matrix(m))
  expect_equal(dim(m), c(2L, 2L))
  expect_identical(colnames(m), c("offset", "length"))
  expect_identical(m[1L, ], c(offset = 0L, length = 100L))
...
test_that("cpp_signature_byte_range errors on an out-of-bounds index", {
  doc <- pdf_doc_open(fixture_path("signed"))
  on.exit(pdf_doc_close(doc), add = TRUE)
  expect_error(
    pdfium:::cpp_signature_byte_range(doc$ptr, 99L),
    "FPDF_GetSignatureObject returned NULL"
  )
})
