...
test_that("pdf_glyph_path returns a non-empty segment tibble for 'H'", {
  ...
  expect_named(gp, c("segment_index", "segment_type", "x", "y",
                     "close_figure"))
  expect_gt(nrow(gp), 0L)
  expect_true(all(gp$segment_type %in% c("moveto", "lineto",
                                          "bezierto", "unknown")))
...
test_that("pdf_glyph_path validates obj type, glyph_code, font_size", {
  ...
  on.exit(pdf_close(bundle$doc), add = TRUE)
  on.exit(pdf_close_page(bundle$page), add = TRUE, after = FALSE)
  expect_error(pdf_glyph_path("nope", 0x48L),
               "must be a `pdfium_obj`")
  expect_error(pdf_glyph_path(bundle$obj, -1L),
               "non-negative integer")
  expect_error(pdf_glyph_path(bundle$obj, NA),
               "non-negative integer")
  expect_error(pdf_glyph_path(bundle$obj, 0x48L, font_size = "12"),
               "single numeric")
  expect_error(pdf_glyph_path(bundle$obj, 0x48L, font_size = c(1, 2)),
               "single numeric")
})
...
test_that("pdf_text_font_metrics validates font_size", {
  ...
  on.exit(pdf_close(bundle$doc), add = TRUE)
  on.exit(pdf_close_page(bundle$page), add = TRUE, after = FALSE)
  expect_error(pdf_text_font_metrics(bundle$obj, font_size = 0),
               "positive finite numeric")
  expect_error(pdf_text_font_metrics(bundle$obj, font_size = NA),
               "positive finite numeric")
})
