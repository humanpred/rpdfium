...
real gaps in the current release:

| Gap | Status | Workaround |
|---|---|---|
| **Image embedding** | `FPDFImageObj_LoadJpegFile` / `FPDFImageObj_SetBitmap` not wrapped yet (deferred from Phase 5 pending `FPDF_BITMAP` plumbing). | Use `minipdf` for image-heavy programmatic PDFs, or render images separately and embed via a post-process. |
| **Custom fonts** | Only the 14 standard PDF fonts; `FPDFText_LoadCidType2Font` not wrapped. | For non-Latin scripts or branded type, render text as paths via an external tool and embed the resulting vector paths. |
| **`/Info` dict writes** | `FPDF_SetMetaText` doesn't exist upstream. We have a [drafted patch](https://github.com/humanpred/rpdfium/blob/main/dev/upstream-patches/pdfium-FPDF_SetMetaText.patch) awaiting upload. | Use `xmpdf` to patch the Info dict after `pdf_save()`. |
| **Encryption on save** | `FPDF_SetEncryption` doesn't exist upstream (tracker CL 5). | Use `qpdf::pdf_encrypt()` as a post-process step. |
...
known path to closure as those upstream patches land.
