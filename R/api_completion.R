...
#' otherwise.
#'
#' Useful when assembling a derived document — concatenation, n-up,
#' page reordering — that should inherit the display and print hints
#' the publisher set on the original.
#'
#' @param dest_doc A `pdfium_doc` opened with `readwrite = TRUE`.
#' @param src_doc Source `pdfium_doc` whose viewer preferences are
#'   read.
#' @return Invisibly returns `dest_doc`.
#' @export
pdf_docs_copy_viewer_preferences <- function(dest_doc, src_doc) {
  assert_readwrite(dest_doc)
  checkmate::assert_class(src_doc, "pdfium_doc")
  if (!is_open(src_doc)) {
    stop("Source document has been closed.", call. = FALSE)
  ...
  expect_setter_ok(
    cpp_doc_copy_viewer_preferences(dest_doc$ptr, src_doc$ptr),
